...
# Load data ----
here_output_files <- here(path_project, "outputs", "models")
df_full_models <- read.csv(here(here_output_files, "models_consolidated_cco_utci_21.csv"))
head(df_full_models)
...
ggsave(here(path_project, "outputs", "figures", "plot_30.jpeg"), plot_30, width = 8, height = 10, dpi = 600)

## For percentile - 90 ----
df_full_models_90 <- df_full_models |> filter(str_detect(exposure, "90"))
head(df_full_models_90)
plot_90 <- func_plot_full_model(df_full_models_90, title = expression("UTCI >= 90th percentile"))

ggsave(here(path_project, "outputs", "figures", "plot_90.jpeg"), plot_90, width = 8, height = 10, dpi = 600)

# Combine the plots ----
plot_combined <- plot_30 + plot_90
# Add tags to the plots
plot_combined <- plot_combined + plot_annotation(tag_levels = "A")
ggsave(here(path_project, "outputs", "figures", "plot_combined.jpeg"), plot_combined, width = 12, height = 8, dpi = 600)
